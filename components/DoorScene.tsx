...
import { FINAL_CHAPTER_TEXT, FINAL_DOOR_ID, DOOR_QUESTS } from '../constants';
...
const DoorScene: React.FC<DoorSceneProps> = ({ doorId, onReturn, onReset }) => {
  ...
  // If no quest data is defined for this door yet, use a fallback
  ...
  // Theme settings based on doorId
  const getContainerStyles = () => {
    if (doorId === '1') {
      // Forest theme window
      return "bg-emerald-950/40 backdrop-blur-xl border-amber-900/30 shadow-[0_0_50px_rgba(6,78,59,0.3)]";
    }
    if (doorId === '2') {
      // Merchant's Tent theme window
      return "bg-red-900/50 backdrop-blur-2xl border-amber-900 border-[6px] shadow-[0_0_60px_rgba(153,27,27,0.4)] rounded-[40px_10px_40px_10px]";
    }
    if (doorId === '3') {
      // Royal Ball theme window
      return "bg-white/90 backdrop-blur-md border-amber-400 border-[4px] shadow-[0_0_40px_rgba(251,191,36,0.3)] rounded-3xl";
    }
    if (doorId === '4') {
      // Seaside theme: Refreshing Teal/Azure with sandy texture hint
      return "bg-teal-900/40 backdrop-blur-xl border-cyan-300/20 shadow-[0_0_50px_rgba(34,211,238,0.2)] rounded-[60px_20px_60px_20px]";
    }
  ...
  const getButtonStyles = () => {
    if (doorId === '1') {
      // Earthy brown buttons
      return "bg-amber-900/60 hover:bg-amber-800/80 border-amber-700/30 text-amber-50 shadow-md";
    }
    if (doorId === '2') {
      // Saturated crimson and gold buttons
      return "bg-red-800/70 hover:bg-red-700/90 border-amber-600/40 text-amber-100 shadow-[0_4px_0_rgba(120,0,0,0.6)] hover:shadow-none";
    }
    if (doorId === '3') {
      // Royal Navy and Gold buttons
      return "bg-indigo-900 hover:bg-indigo-800 border-amber-500/50 text-amber-100 shadow-md transform hover:scale-105";
    }
    if (doorId === '4') {
      // Sea Azure and Sandy Gold buttons
      return "bg-cyan-700/60 hover:bg-cyan-600/80 border-cyan-400/30 text-cyan-50 shadow-lg hover:shadow-cyan-400/20";
    }
    ...
    return (
      <div className="w-full max-w-2xl flex flex-col items-center">
        <div className={`w-full border transition-all duration-300 ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'} ${getContainerStyles()} relative overflow-hidden`}>
          {/* Subtle tent decorative elements for Door 2 */}
          {doorId === '2' && (
              ...
              {currentStep.isEnd ? (
                <button 
                  ...
                  className={`px-12 py-4 rounded-full font-bold text-lg transition-all transform hover:-translate-y-1 shadow-lg ${
                    doorId === '1' 
                    ? 'bg-gradient-to-r from-emerald-600 to-amber-800 hover:from-emerald-500 hover:to-amber-700 text-white shadow-emerald-900/40' 
                    : doorId === '2'
                    ? 'bg-gradient-to-r from-red-700 to-amber-900 hover:from-red-600 hover:to-amber-800 text-amber-50 shadow-red-950/60'
                    : doorId === '3'
                    ? 'bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-indigo-950 shadow-amber-900/20'
                    : doorId === '4'
                    ? 'bg-gradient-to-r from-cyan-600 to-blue-800 hover:from-cyan-500 hover:to-blue-700 text-white shadow-cyan-950/40'
